...
const OrderDesc = () => {
  ...
  const { backend, token, calculateDelivery,
    calculateGst,
    calculatePlatformFee, } = useContext(AppContext);
  ...
  const generateInvoice = async () => {
   ...
   // 🔹 Calculate totals based on THIS order, not cart
    let totalItemsAmount = 0;
    let totalQty = 0;

    order.items.forEach((item) => {
      totalItemsAmount += item.newprice * item.quantity;
    ...
    if (totalQty > 0) {
      if (totalQty <= 10) {
        ...
        deliveryFee = 39 + (totalQty - 10) * 10;
      }
    }

    // Platform Fee (7%)
    const platformFee = Math.round(totalItemsAmount * 0.07);

    // GST (5%)
    const gstFee = Math.round((totalItemsAmount + platformFee + deliveryFee) * 0.05);

    // Final Total
    const finalTotal = totalItemsAmount + platformFee + deliveryFee + gstFee;
      ...
      addCharge("Platform Fee", platformFee);
      addCharge("Delivery Fee", deliveryFee);
      addCharge("GST (5%)", gstFee);
        ...
        <div className="mt-5 w-full lg:w-[90%] xl:w-[80%]">
          ...
          <div className="mt-10 w-full flex flex-wrap gap-5 justify-between">
            ...
            <div className="flex gap-2 items-center">
              ...
              <span
                className={`text-base font-medium text-zinc-800 ${order.payment
                  ? "text-green-600"
                  : order.paymentType === "Cash On Delivery"
                    ? "text-yellow-600"
                    : "text-red-600"
